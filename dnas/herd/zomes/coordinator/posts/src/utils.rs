...
pub fn get_latest_record(original_ah: ActionHash) -> ExternResult<Record> {
    let details = get_details(original_ah, GetOptions::latest())?.ok_or(wasm_error!(
        WasmErrorInner::Guest("Record with that original action hash not found".into())
    ))?;
        ...
        Details::Record(record_details) => {
            match record_details.deletes.len() {
                ...
                0 => {
                    match record_details.updates.len() {
                        ...
                        _ => {
                            ...
                            let latest_update_record =
                                get(latest_update.clone().hashed.hash, GetOptions::latest())?
                                    .ok_or(wasm_error!(WasmErrorInner::Guest(
                                        "Latest update record not found".into()
                                    )))?;
                ...
                _ => {
                    ...
                    let earliest_delete_record =
                        get(earliest_delete.clone().hashed.hash, GetOptions::latest())?.ok_or(
                            wasm_error!(WasmErrorInner::Guest(
                                "Earliest delete record not found".into()
                            )),
                        )?;
