...
#[hdk_extern]
pub fn get_post_metadata(original_post_hash: ActionHash) -> ExternResult<Option<PostMetadata>> {
    ...
    if let Some(post) = maybe_post {
        ...
        let latest_post_vote_tags: Vec<PostVoteTag> = post_vote_links
            .into_iter()
            .map(|link| PostVoteTag::try_from(SerializedBytes::from(UnsafeBytes::from(link.tag.0))))
            .filter_map(Result::ok)
            .collect();

        // Count upvotes
        let upvotes = latest_post_vote_tags.clone()
            .into_iter()
...
pub fn upvote_post(original_post_hash: ActionHash) -> ExternResult<()> {
    ...
    create_link(agent_pubkey.clone(), original_post_hash.clone(), LinkTypes::MyUpvotedPosts, ())?;
    create_link(original_post_hash, agent_pubkey, LinkTypes::PostVoteByAgent, make_post_vote_tag(1)?)?;
...
pub fn downvote_post(original_post_hash: ActionHash) -> ExternResult<()> {
    create_link(original_post_hash, agent_info()?.agent_initial_pubkey, LinkTypes::PostVoteByAgent, make_post_vote_tag(-1)?)?;
...
pub fn make_post_vote_tag(value: i8) -> ExternResult<LinkTag> {
    let tag_bytes = SerializedBytes::try_from(PostVoteTag { value })
        .map_err(|_e| wasm_error!(WasmErrorInner::Guest("serializedbytes error".into())))?;
